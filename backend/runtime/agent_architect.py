...
====================
Converts planner DAG nodes into executable AgentSpec objects.
"""
...
from runtime.agents.base_agent import get_primary_llm, invoke_with_retry
...
def _build_spec_from_node(node: dict, index: int) -> dict:
    """Build an AgentSpec directly from a DAG node when LLM refinement is unavailable."""
    return {
        ...
        "capability_rationale": "Built directly from planner DAG node without LLM refinement",
    }
...
async def agent_architect_node(state: PlatformState) -> dict:
    ...
    for node in nodes:
        ...
        valid_caps = [cap for cap in original_caps if cap in available_caps]
    ...
    if llm:
        try:
            ...
            for spec in agent_specs:
                ...
                print(
                    f"  -> {spec['agent_id']} | template={spec['template']} | "
                    f"caps={len(caps)} | model={spec.get('model')}"
                )
        except Exception as exc:
            print(f"[AGENT_ARCHITECT] LLM spec refinement failed ({exc}), building specs from DAG nodes directly")
